...
type Copy = {
  ...
  glossaryLabel: string; glossaryTitle: string; glossaryIntro: string; glossary: [string, string][];
  localLabel: string; privacyLink: string; publicLabel: string;
};
...
const COPY: Record<Locale, Copy> = {
  pt: {
    ...
    mission: "IA melhor não pode ser privilégio de quem paga mais.", missionBody: "Batuta é infraestrutura pública, gratuita e sem lucro. Uso profissional de IA compreensível para iniciantes e mensurável para especialistas.", manifest: "Ler o manifesto", metrics: ["skills com resultado publicado", "receitas publicadas", "tarefas na bateria v1", "teto medido no corpus v1"], source: "ver fonte", glossaryLabel: "Glossário", glossaryTitle: "Começando agora? Três palavras bastam.", glossaryIntro: "Você não precisa conhecer o vocabulário inteiro para usar IA melhor.", glossary: [["Agente", "Uma IA que recebe um objetivo e executa etapas para alcançá-lo."], ["Skill", "Uma instrução especializada que ensina o agente a fazer melhor uma tarefa."], ["Modelo", "O motor de IA. Modelos variam em capacidade, velocidade e preço."]], localLabel: "Local primeiro", privacyLink: "Privacidade desde o projeto", publicLabel: "Bem público",
  },
  en: {
    ...
    mission: "Better AI cannot be a privilege for those who pay more.", missionBody: "Batuta is free, nonprofit public infrastructure. Professional AI should be understandable for beginners and measurable for experts.", manifest: "Read the manifesto", metrics: ["skills with published results", "published recipes", "tasks in benchmark v1", "measured limit on corpus v1"], source: "view source", glossaryLabel: "Glossary", glossaryTitle: "New here? Three words are enough.", glossaryIntro: "You do not need to learn the whole vocabulary to use AI better.", glossary: [["Agent", "An AI that receives a goal and carries out steps to achieve it."], ["Skill", "Specialized instructions that teach an agent to perform one task better."], ["Model", "The AI engine. Models differ in capability, speed and price."]], localLabel: "Local first", privacyLink: "Privacy by design", publicLabel: "Public good",
  },
  es: {
...
export function HomeContent({ skillsMeasured }: { skillsMeasured: number }) {
  ...
  return <div>
    ...
    <section className="faixa home-body"><div className="centro home-wide">
      ...
      <section className="glossary"><div><p className="section-kicker">{c.glossaryLabel}</p><h2>{c.glossaryTitle}</h2><p>{c.glossaryIntro}</p></div><dl>{c.glossary.map(([term, definition]) => <div key={term}><dt>{term}</dt><dd>{definition}</dd></div>)}</dl></section>
      <NewsletterForm />
      <div className="statement-grid"><article><p className="section-kicker">{c.localLabel}</p><h2>{c.today}</h2><p>{c.todayBody}</p><a href="/privacidade">{c.privacyLink} →</a></article><article className="mission"><p className="section-kicker">{c.publicLabel}</p><h2>{c.mission}</h2><p>{c.missionBody}</p><a className="botao" href="/manifesto">{c.manifest}</a></article></div>
    </div></section>
  </div>;
}
